      ...
      <div class="main">
            <div class="info">
                ...
                <div class="choose">
                    ...
                    <div class="choosebg">
                        <span v-for="item in payModes" 
                        :key="item.code" @click="tabPayWay(item)"
                        >{{item.description}}</span>
                    </div>
                </div>
                <ul class="foot">
                    <li class="foot-item">应付<span class="unique">￥{{orderInfo.totalPrice}}</span></li>
                    <li>
                        <button class="btn" @click="createOrder">确认订单</button>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <el-dialog v-model='dialogVisible' class="pay-dialog" width="500px">
            <div class="dialogPrice">支付：<span class="prices">{{orderInfo.totalPrice}}元</span></div>
            <div class="codeimg">
                <img :src="payURL" />
            </div>
...
import { settlement,zfbpayOrder,queryZfbOrder,queryWxOrder,wxpayOrder,deleteShopCars} from '../utils/api/cart'
...
onBeforeMount(()=>{
   //请求结算商品api的参数 cartStore.orderList
...
// 查询支付宝订单状态
const interPaymentZfb=()=>{
    // 查案[支付宝]支付订单状态
queryZfbOrder({
    orderNumber:payNumber.value
}).then(res=>{
       if(res.meta.code=='200'){
        ...
        createToken().then(res=>{
            ...
            deleteShopCars(cartStore.select,token).then(res=>{
                console.log(res);
            })
        })
...
const tabPayWay=(item)=>{
    ...
    if(item.code=='alipayment'){
        ...
        console.log(res);
        payNumber.value=res.data.orderNumber
...
const createOrder=()=>{
    if(payment.code=='alipayment'){
        timer.value=setInterval(interPaymentZfb,2000)//两秒查看一次状态
    }else{
